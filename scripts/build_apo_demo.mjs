...
(function () {
  ...
  var DEMO_STAFF = [
    ...
    { name: "営業四郎", email: "eigyo4@example.com", role: "営業" }
  ];
  ...
  /* 稽古用の日付。**1日目は必ず「今日」**にする。土日を飛ばす作りにしたら、土曜に開いた
     ときに本日が0件になり、デモが空っぽで開いた(2026-08-22 実機で検出)。
     2日目以降は土日を飛ばす。土日が空くことで「全員終日あき」の日も自然に混ざる。 */
  var DEMO_DAYS = (function () {
    var days = [dateString(new Date())];
    var cursor = new Date();
    while (days.length < 5) {
      cursor = new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() + 1);
      var day = cursor.getDay();
      if (day !== 0 && day !== 6) { days.push(dateString(cursor)); }
    }
    return days;
  })();

  var demoSeq = 0;
  function seed(dayIndex, time, minutes, customer, format, place, sales, temp, status, reason) {
    demoSeq += 1;
    var rec = {};
    rec["アポID"] = "APO-DEMO-" + pad2(demoSeq) + pad2(dayIndex);
    rec["日付"] = DEMO_DAYS[dayIndex]; rec["開始時刻"] = time;
    rec["所要分"] = minutes; rec["顧客名"] = customer; rec["形式"] = format;
    rec["場所またはURL"] = place; rec["担当営業"] = sales; rec["アポ入れ担当"] = "アポ花子";
  ...
  /* 稽古用の初期データ(顧客名・スタッフ名はすべて架空)。
   *
   * うまくいっている状態だけ並べても設計の穴は出ないので、**わざと困った状態を混ぜてある**:
   *   ・営業二郎の[2日後] … 時間が重なっている(手当てが要るものに出る)
   *   ・営業三郎の[本日]   … 差し戻しが続いて午前がまるごと空いた
   *   ・営業四郎の[3日後] … 1人だけ終日空き
   *   ・[1日後]           … 行き先が空のアポ / 担当営業が空のアポ
   *   ・[本日]の午前       … 結果が入っている行と入っていない行の両方
   * ステータス9種すべてを1件以上含む。90分以上の空きが自然に残る時間配置にしてある。 */

  // ===== 本日 =====
  seed(0, "09:00", 60, "サンプル商店 田中様", "訪問", "那覇市おもろまち1-2-3", "営業一郎", "高", "訪問済");
  seed(0, "10:30", 60, "テスト物産 比嘉様", "訪問", "宜野湾市真志喜2-4-5", "営業一郎", "高", "申込");
  seed(0, "13:00", 90, "見本工業 金城様", "来店", "本社2F", "営業一郎", "中", "アポ確定");
  seed(0, "15:30", 60, "架空建設 大城様", "訪問", "浦添市牧港3-1-1", "営業一郎", "低", "アポ確定");
  seed(0, "09:30", 60, "例示食品 宮城様", "オンライン", "Zoom", "営業二郎", "高", "成立");
  // 隣り合わせだが重なってはいない(ダブルブッキング寸前。移動時間が取れるかを見る場面)
  seed(0, "11:00", 60, "仮設商事 新垣様", "訪問", "那覇市久茂地1-1-1", "営業二郎", "中", "訪問済");
  seed(0, "12:00", 60, "模擬電機 玉城様", "訪問", "那覇市泉崎2-2-2", "営業二郎", "中", "アポ確定");
  // 差し戻しが続いて午前がまるごと空いた日
  seed(0, "09:00", 60, "試作産業 島袋様", "訪問", "沖縄市中央1-2-3", "営業三郎", "中", "差し戻し", "顧客都合");
  seed(0, "11:00", 60, "見本商会 平良様", "訪問", "うるま市石川東恩納", "営業三郎", "高", "差し戻し", "自社都合");
  seed(0, "14:00", 90, "ダミー興産 上原様", "訪問", "北谷町美浜9-1", "営業三郎", "高", "アポ確定");
  seed(0, "13:00", 60, "例題運輸 山城様", "来店", "本社2F", "営業四郎", "中", "追客中");
  seed(0, "16:00", 60, "架空印刷 又吉様", "オンライン", "Meet", "営業四郎", "低", "保全中");

  // ===== 1日後: 行き先が空 / 担当が空 / 未確定が多い =====
  seed(1, "10:00", 60, "サンプル製菓 仲宗根様", "訪問", "", "営業一郎", "高", "アポ確定");
  seed(1, "13:30", 90, "テスト工房 具志堅様", "訪問", "南城市玉城", "営業一郎", "中", "スケジュール調整中");
  seed(1, "09:30", 60, "模型設備 知念様", "来店", "本社2F", "営業二郎", "中", "アポ確定");
  seed(1, "15:00", 60, "仮想物流 照屋様", "訪問", "豊見城市豊崎", "営業二郎", "高", "スケジュール調整中");
  seed(1, "11:00", 60, "例示塗装 池宮様", "訪問", "糸満市西崎", "営業三郎", "低", "アポ確定");
  seed(1, "14:30", 60, "架空養鶏 儀間様", "訪問", "南風原町兼城", "営業三郎", "中", "スケジュール調整中");
  // 担当営業が空(シートを直接いじると起きる。黙って消さず「(担当なし)」で出す)
  seed(1, "16:00", 60, "見本電設 与那覇様", "訪問", "西原町小那覇", "", "中", "スケジュール調整中");
  seed(1, "10:30", 90, "試験農園 喜屋武様", "訪問", "八重瀬町東風平", "営業四郎", "高", "アポ確定");

  // ===== 2日後: 時間が重なっている(手当てが要るものに出る) =====
  seed(2, "09:00", 60, "サンプル鉄工 安里様", "訪問", "那覇市壺川", "営業一郎", "中", "アポ確定");
  seed(2, "14:00", 60, "模擬酒造 宮里様", "来店", "本社2F", "営業一郎", "高", "アポ確定");
  seed(2, "10:00", 120, "例題ホテル 神谷様", "訪問", "恩納村谷茶", "営業二郎", "高", "アポ確定");
  seed(2, "11:00", 60, "仮称薬局 渡嘉敷様", "訪問", "読谷村喜名", "営業二郎", "中", "アポ確定");
  seed(2, "13:00", 60, "架空水産 高良様", "訪問", "糸満市西崎町", "営業三郎", "中", "アポ確定");
  seed(2, "15:30", 60, "見本製糖 城間様", "オンライン", "Zoom", "営業三郎", "低", "スケジュール調整中");
  seed(2, "09:30", 90, "試作繊維 座間味様", "訪問", "浦添市港川", "営業四郎", "中", "アポ確定");
  seed(2, "13:30", 60, "例示造園 東江様", "訪問", "宜野湾市大山", "営業四郎", "高", "アポ確定");

  // ===== 3日後: 営業四郎だけ終日空き =====
  seed(3, "09:00", 90, "サンプル介護 譜久村様", "訪問", "沖縄市高原", "営業一郎", "高", "アポ確定");
  seed(3, "13:00", 60, "模擬印刷 我如古様", "来店", "本社2F", "営業一郎", "中", "アポ確定");
  seed(3, "15:30", 60, "テスト保育 銘苅様", "訪問", "那覇市銘苅", "営業一郎", "低", "スケジュール調整中");
  seed(3, "10:00", 60, "架空整備 津波古様", "訪問", "南城市佐敷", "営業二郎", "中", "アポ確定");
  seed(3, "14:00", 90, "例題観光 保栄茂様", "訪問", "北中城村喜舎場", "営業二郎", "高", "アポ確定");
  seed(3, "11:30", 60, "仮設精肉 金武様", "訪問", "金武町金武", "営業三郎", "中", "アポ確定");
  // 架電禁止・アポ禁(❶から連携が通るまでは手で設定する運用)
  seed(3, "16:00", 60, "見本商店 越来様", "訪問", "沖縄市越来", "営業三郎", "低", "対象外");

  // ===== 4日後 =====
  seed(4, "09:30", 60, "試験電気 名嘉様", "訪問", "うるま市赤道", "営業一郎", "中", "アポ確定");
  seed(4, "13:00", 90, "サンプル寝具 前泊様", "訪問", "那覇市小禄", "営業二郎", "高", "アポ確定");
  seed(4, "10:00", 60, "模型工務 仲村渠様", "来店", "本社2F", "営業三郎", "中", "アポ確定");
  seed(4, "14:30", 60, "架空クリーニング 屋良様", "訪問", "嘉手納町屋良", "営業三郎", "低", "スケジュール調整中");
  seed(4, "11:00", 60, "例示花卉 平安名様", "訪問", "宮古島市平良", "営業四郎", "中", "アポ確定");
  seed(4, "15:00", 90, "仮想書店 桃原様", "来店", "本社2F", "営業四郎", "高", "アポ確定");
