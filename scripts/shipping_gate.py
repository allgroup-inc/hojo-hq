...
`scripts/check_humanizer.py` との違い(2026-08-17 両方が同日に入ったため明記する):
  check_humanizer.py = 文体のAIっぽさ(定型句・絵文字過多・反復)を検査し **報告のみ・止めない**。
                       誤検知で投稿が止まる方が害が大きいという決裁(決裁キュー14.c)に基づく。
  shipping_gate.py   = 正確性・法務・計測を守る検査で **止める**。事実誤り(締切7日前)・断定・
                       lin.ee直貼りは誤検知しにくく、外に出た場合の害が文体より大きい。
  両者は重複ではなく、止めてよいものと止めてはいけないものの線引きで分かれている。
...
FORBIDDEN = (
    ...
    (r"後継者\s*・?\s*M\s*&\s*A\s*の\s*お?悩み|M&A\s*の\s*(?:ご)?相談|承継\s*の\s*お?悩み",
     "承継・M&Aニーズへの直接訴求。カモフラージュ設計(2026-08-11)違反 — "
     "制度名としての「事業承継〜補助金」の記載は可、悩み・相談への誘い文句は不可"
     "(2026-08-29 小柳さん指摘: SNSキャプションに残存していたため機械検査化)"),
    (r"#\s*(?:事業承継|沖縄M&A|後継者問題|M&A)\b",
     "承継・M&A系ハッシュタグ。カモフラージュ設計違反(同上)"),
)
