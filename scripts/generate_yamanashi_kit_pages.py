...
function fkCopySheet(btn){ fkCopy(FK_SHEET_TEXT, btn, "ymn_kit_copy"); }
function fkCopySay(btn){
  var el = document.getElementById("mado-say");
  fkCopy(el ? el.textContent.trim() : "", btn, "ymn_kit_copy_say");
}
(function(){
  var KEY = "fk_ymn_kit___ID__";
  var saved = {c:[], m:{}, s:[]};
  ...
  boxes.forEach(function(b, i){
    ...
    b.addEventListener("change", function(){
      save(); updItems();
      if (!trackedCheck && window.fgTrack) { trackedCheck = true; fgTrack("ymn_kit_check"); }
    });
  });
  steps.forEach(function(b, i){
    ...
    b.addEventListener("change", function(){
      save(); updSteps();
      if (!trackedStep && window.fgTrack) { trackedStep = true; fgTrack("ymn_kit_step"); }
    });
  });
...
def kit_page(it, updated):
    ...
    madoguchi_html = ""
    mg = None  # 全国の制度のみのため「聞きに行く場所と時間」ブロックは出さない(市町村制度の追加時に沖縄版と同様の仕組みを入れる)
    if mg:
        ...
        madoguchi_html = (
            ...
            'onclick="if(window.fgTrack)fgTrack(\'ymn_kit_map\')">地図アプリでひらく</a></p>'
            f'<p style="margin-top:8px">☎ <a href="tel:{tel_digits}" onclick="if(window.fgTrack)fgTrack(\'ymn_kit_tel\')">{esc(mg["tel"])}</a>(代表)'
    ...
    _sub = urllib.parse.quote(f"[情報の訂正・山梨] {it['name']}")
...
<div class="btns">
  <button class="primary" onclick="if(window.fgTrack)fgTrack('ymn_kit_print');window.print()">印刷する</button>
  <a href="{src}" rel="noopener" onclick="if(window.fgTrack)fgTrack('ymn_kit_official')">公式ページで最新を確認</a>
</div>
...
<div class="after">
  ...
  <p class="note" style="margin-top:6px">・結果の通知や受け取りの時期は、④のメモで管理できます。<br>・「受け取れた」まで確認できたら、下から教えてください。次のご家庭の「もらい忘れ」を防ぐ力になります。</p>
...
"""
    ...
    elif area == "山梨県" and not it["name"].startswith("山梨"):
        title = f"山梨県 {it['name']}の申請方法・持ち物・窓口|もらいわすれ堂"
    elif area in MUNI_SLUG or area == "山梨県":
        title = f"{it['name']}の申請方法・持ち物・窓口|もらいわすれ堂"
    else:
        title = f"{it['name']}の申請方法・持ち物・窓口(山梨)|もらいわすれ堂"
    ...
    crumbs = breadcrumb_jsonld([("もらいわすれ堂 山梨版", ""), ("申請準備シート", "kit/"), (it["name"], None)])
...
"""
    return page(
        "山梨の給付金・手当の申請準備シート一覧(持ち物・窓口・電話の聞き方)|もらいわすれ堂",
        "山梨の給付金・手当の申請準備シート一覧。制度ごとに持ち物リスト・窓口での聞き方・電話で確認する3つのことをまとめ、印刷してそのまま窓口へ持っていけます(無料)。",
        body, depth=1,
        head_extra=breadcrumb_jsonld([("もらいわすれ堂 山梨版", ""), ("申請準備シート", None)]),
        canon_path="kit/")
...
def main():
    ...
    print(f"生成完了(山梨版): 申請準備シート{len(items)}ページ+一覧1ページ")
