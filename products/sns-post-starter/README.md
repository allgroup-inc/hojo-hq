...
Claude APIが各トピックから
  ・キャプション(Instagram用・検索キーワード織り込み済み)
  ・短文版(X用・140字チェックつき)
  ・ハッシュタグ(内容直結の3〜5個のみ)
  ・カルーセル下書き(手順・リスト系トピックのみ自動判定)
  ・画像のアイデア
...
## 2026年の「見つけてもらう」設計になっています

このテンプレートの生成ロジックは、古い「ハッシュタグを30個つける」式では
なく、いまのSNSの発見の仕組みに合わせてあります。

- **検索キーワード優先(ソーシャルSEO)**: いまのSNSは検索エンジンです。
  読者が実際に検索する言葉(`brand.yml` の `seo_keywords`)をキャプション
  冒頭に自然に織り込みます。発見への効きはハッシュタグより上です
- **ハッシュタグは3〜5個だけ**: Instagramは実質5個までしか効きません
  (2025年〜)。固定タグ+生成タグを自動で5個以内に整理し、#love のような
  汎用メガタグは生成させません
- **冒頭1行=フック**: 約125字で「…続きを読む」に切られる前提で、内容に
  忠実な範囲で最も強い1行(数字・驚き・共感)を冒頭に置きます
- **媒体別チェック**: Xの140字超過・Instagramのキャプション上限・タグ数を
  プログラムが自動検査し、引っかかった投稿には「要修正」フラグが付きます
- **カルーセル自動判定**: 手順・リスト・ビフォーアフター系のトピックは、
  保存されやすいカルーセル(表紙フック+1枚1メッセージ+保存CTA)の
  スライド文言も一緒に生成します
