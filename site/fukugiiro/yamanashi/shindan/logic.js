/* フクギイロ 世帯診断ロジック v1 山梨版(ミチビキ)— 沖縄版 site/fukugiiro/shindan/logic.js の複製。県名のみ変更
 * - ブラウザ(window.FGShindan)とNode(module.exports)の両方で動くUMD形式。
 *   自動テストは沖縄版(tests/shindan.test.mjs)が同一ロジックを検証している。
 *   山梨版で判定条件を変えるときは、山梨版用のテストを先に用意すること。
 * - 方針(診断フロー設計v1・守り部):
