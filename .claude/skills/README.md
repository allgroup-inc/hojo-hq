...
- `resilient-agent-design` — GitHub Actions / cron / Claude Routines / `/loop` など無人稼働する自動化・AIエージェントを新規設計/レビューするとき、または「壊れる・重複する・止まらない・原因不明」のトラブル時に使う。4層設計(Trigger/Workflow/Agent/Guardrail)・7つの設計原則・実行方式の選び方を提供。既存自動化(cron→Pythonスクリプト→seido.json等に保存→--self-testで検証→異常時Issue起票＋LINE通知)の型に揃える。
