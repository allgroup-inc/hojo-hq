...
- `humanizer` — SNS投稿・LINE配信・サイトコピーの仕上げで必ず使う。AIっぽい定型フレーズ・過剰な絵文字/強調を除き、自然な日本語に整える(事実情報は変更しない)。
- `resilient-agent-design` — GitHub Actions/cron/Routines/`/loop`等の自動化を新規設計・レビューするとき。壊れにくい自動化の7原則(状態外部化・べき等性・リトライ設計・最小権限等)を強制。`weekly-report.yml`の重複LINE配信事故(2026-08-03)を教訓として反映。
