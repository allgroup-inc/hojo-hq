...
4. 即時アラートを有効にするため、Apps Scriptエディタで `installInteractionLogEditTrigger`
   を一度だけ実行する(初回は権限承認のダイアログが出るので許可する)。この関数は
   `対応履歴ログ`の編集を検知するインストール型トリガーを登録する。**二重登録を防ぐため、
   再実行する前にApps Scriptエディタの「トリガー」画面で既存のトリガーがないか確認すること**
...
セットアップ手順4で登録したインストール型トリガーが `handleInteractionLogEdit` を
呼び出し、即座にSlackへ通知する。この関数名はGASの予約語ではないため、
`onEdit`という名前の関数を別途定義しても衝突しない。
