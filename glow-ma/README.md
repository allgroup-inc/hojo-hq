...
- **塩漬け企業一覧**(`alerting.isStale` / `alerting.buildStaleList`): 既存の`isOverdue`
  (単発の期限超過)とは別に、標準サイクルの2倍(`staleMultiplier`)以上、最終接触が
  無い企業を検出する。次回アクション予定日が設定されていても、実際の最終接触が
  長期間無ければ塩漬け対象とする(予定日を設定しただけで放置され続けるケースを拾うため)
- ダッシュボード履歴に「塩漬け企業数」を追加し、推移を追跡できるようにした
- 三名体制の議事は
...
入電の把握・担当者別ワークロード/塩漬け企業の検知)はすべて実装済みです。
