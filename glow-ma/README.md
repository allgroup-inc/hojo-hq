...
> **明示的に宣言**した(権限は最小限からの原則により、Drive権限はDrive全体を読み書きできる
> `auth/drive` ではなく、スクリプト自身が作成したファイル・フォルダだけにアクセスできる
> `auth/drive.file` に絞っている)。現在の宣言は `drive.file` / `spreadsheets` /
> `script.external_request` / `script.scriptapp` / `script.container.ui` /
> `userinfo.email` の6件(`script.container.ui` はメニュー・ダイアログ・サイドバーの
> 表示に必須。本プロジェクトはメニューUI前提のため、欠けると全機能が使えなくなる)。
>
...
- QR画像の保存先フォルダは、スクリプトプロパティ `QR_FOLDER_ID_<発送日>` に記録した
  フォルダIDで参照する(名前での検索は `drive.file` 権限下では確実に動く保証がないため)。
  フォルダをゴミ箱に入れた・IDが古い場合は、名前検索 → 新規作成の順にフォールバックする

**デプロイ後のスモークテスト(重複フォルダ検知):**

`drive.file` 権限下でのDrive参照は本番環境でしか最終確認できない。デプロイ後、
