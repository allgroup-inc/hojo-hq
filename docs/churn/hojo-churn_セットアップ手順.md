...
- **保持年数 = 6年**（`config.SNAPSHOT_RETENTION_YEARS`。小柳さん指示 2026-08-11）。
  ※**適法性は守り部・外部専門家レビュー待ち**（#142 項目3）。確定するまで実データの実削除は運用しない。
- 削除ジョブ実装済み: `scripts/churn/retention.py`（CLI `retention`）。
  - **既定はドライラン**（消さない・満期一覧を出すだけ）。実削除は `--apply` を明示したときだけ。
  - べき等（既に無い月は対象外）・**監査ログ** `deletion_log.json`（月・行数・sha256・削除基準日＝PII非出力）。
  - 満期判定はカレンダー基準（取得月 + 6年 で満期）。
- 運用例（統制環境・審査後）:
  ```
  # まず何が消えるか確認
  python -m scripts.churn.cli retention --dir private/snapshots --as-of <当日>
  # 専門家レビューで年数確定後、実削除
  python -m scripts.churn.cli retention --dir private/snapshots --as-of <当日> --apply
  ```
- バックアップ側の削除確認（複製先でも満期削除されるか）は守り部が運用に含める。
