...
このプランのタスクはすべてコードの実装であり、以下の外部アカウント作成・設定はユーザー側で行う必要がある。**最終レビューでこの手順の抜けが複数指摘され(下記に反映済み)、コードの完成度に対して手順の抜けが最大のリスクになっている。**
...
2. **Google側のGASプロジェクト作成**
   ...
   - 「デプロイ」→「新しいデプロイ」→種類「ウェブアプリ」、**「次のユーザーとして実行」は「自分」を選択**(これを怠るとSheets/MailAppへの匿名POSTからのアクセスが失敗する)、アクセスできるユーザーを「全員」にしてデプロイし、発行されたWeb App URLを控える
   - 初回デプロイ時、Sheets/Gmail/UrlFetchの権限スコープに対する**OAuth初回認可**を求められるので承認する
   - **重要**: デプロイを「新しいデプロイ」として作り直すと、Web App URLが変わる。以後のコード更新は既存デプロイを「編集」して再デプロイし、URLを変えないこと。URLが変わると、Stripe側に登録したWebhook URLとフロント側に埋め込んだURL(手順4)の両方が壊れ、気づきにくい形で決済〜レポート配信が止まる
   - GASの`MailApp`は無料枠が1日あたり約100通(消費者向けGoogleアカウントの場合)。送信元アドレスもGASプロジェクトの所有者のGmailアドレスになる。件数が増えてきたら送信基盤の見直しを検討する
...
4. **フロント側のURL差し替え**
   ...
   - **本番化(手順6)の前に、このプレースホルダーが差し替わっていることを必ず目視確認する**。差し替え忘れたまま公開すると、決済ボタンが機能しないページが本番で見える状態になる

5. **手動E2E確認**(Stripeテストモードで)
   - 診断フォームに回答→テストカード番号で決済→completeページに遷移することを確認
   - **Stripeダッシュボードでwebhookが2xxを返していることを確認する。GAS Web AppはHTTPリダイレクト(302)を返す実装になっており、Stripeがこれを配信失敗とみなして再送を繰り返す可能性がある。** もし2xxが返らない場合、`handleStripeWebhook`側の冪等性ガード(`reportStatus === "sent"`なら即座に`{ok:true, duplicate:true}`を返す)により再送自体は無害化されているが、Stripe側の再送スケジュール(最大3日間)によりダッシュボード上でエラー表示が続く可能性がある。頻発する場合は、Webhook受付を「トークン検証→Sheetsに受付だけ記録して即応答→レポート生成は時間主導トリガーで別途処理」という非同期構成に切り替えることを検討する(この切り替えは本プランのスコープ外・別途設計が必要)
   - Sheetsの該当行が`paid`・`sent`に更新されていることを確認
   - 入力したメールアドレスにレポートメールが届くことを確認
   - 意図的に2回連続でwebhookを送信(Stripeダッシュボードの「再送信」機能等)し、メールが1通しか届かないこと・Claude API呼び出しが2回走らないことを確認する(冪等性ガードの動作確認)

6. **本番化**
   - 小柳さんの掲載決裁が出たら、`.github/workflows/update.yml`の除外ステップを外し、Stripeを本番モード(`sk_live_...`)に切り替える
   - `site/nobishiro/contact/index.html`・`analytics-config.js`の`contact@example.com`プレースホルダーを実際の問い合わせ用メールアドレスに差し替える(Plan 1由来の既知の課題。有料商品の決済後サポート窓口として機能する必要があるため、本番化の前提条件とする)
   - **特定商取引法に基づく表記(`site/nobishiro/tokushoho/`)・プライバシーポリシー(`site/nobishiro/privacy/`)はテンプレートを作成済み。** フッターおよび診断フォームの申込ボタン直上から購入前に閲覧できる位置に設置してある。ページ内の赤字【要記入】箇所(事業者名・所在地・電話番号・問い合わせ先メール・返品/キャンセル方針)に実際の情報を入れ、小柳さんの確認を受けてから`noindex`を外す(絶対ルール2の法務遵守に該当)

## 既知のリスク(v1で意図的に許容したトレードオフ)

- **`?type=submit`エンドポイントは無認証**: 誰でも診断フォームのSheets行とStripe Checkout Sessionを作成できる(Stripe自体は本人確認なしにセッションを作れても課金は発生しないため金銭的リスクは低いが、カチカクくんが営業に使うリード台帳を汚染しうる。また、GASの`UrlFetchApp`には日次クォータがあり、大量リクエストで枯渇させられる可能性がある)。v1では実装しない。悪用が実際に観測された場合は、メールアドレスごとの1日あたり件数上限をSheets参照でチェックする等の軽量な対策を追加する
- **Stripe Checkout Session作成が失敗した場合**、`handleSubmit`で先に追記したSheetsの行は`paymentStatus: "pending"`のまま残り、自動クリーンアップの仕組みはない(リードとして残しておく設計判断として許容)
