# Okinawa M&A Executive Presentation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate a 12-page executive PowerPoint presentation on Okinawa's M&A market with competitive analysis, risk assessment, entrepreneur psychology, and GLOW strategic positioning.

**Architecture:** Single pptxgenjs Node.js script that generates all 12 slides with Wantedly-style minimal design (white background, orange #FF6B35 and navy #003366 accents only). Data sourced from existing research document and general business/psychology knowledge.

**Tech Stack:** 
- pptxgenjs (Node.js PowerPoint library)
- Bash for execution and file management
- Git for versioning

## Global Constraints

- **Format:** 12 pages PowerPoint (LAYOUT_16x9 = 10" × 5.625")
- **Design:** Wantedly minimalist (white background, orange/navy accents only, large fonts: titles 40-54pt, body 12-16pt)
- **Content source:** Existing market research data + general business/psychology knowledge (NO hypotheses)
- **Color palette:** Orange #FF6B35, Navy #003366, White #FFFFFF, Light Gray #F5F5F5, Dark Gray #333333
- **Font:** Arial throughout
- **Output:** `/tmp/okinawa_ma_executive.pptx` → copy to `/home/user/hojo-hq/docs/`
...
**Create:**
- `/root/.claude/skills/pptx/create_okinawa_ma_executive.js` — Single pptxgenjs script generating all 12 slides

**Output:**
- `/home/user/hojo-hq/docs/okinawa_ma_executive.pptx` — Final presentation file

---

## Task Breakdown

### Task 1: Set Up Script Structure & Color System

**Files:**
- Create: `/root/.claude/skills/pptx/create_okinawa_ma_executive.js`

**Interfaces:**
- Produces: Node.js script with PptxGenJS instance, color palette, helper utilities

**Steps:**

- [ ] **Step 1: Create script header with requires and initialization**
...
Check that the file has been created at the correct path and contains the initialization code.

- [ ] **Step 3: Commit**

```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: scaffold executive presentation script with color system"
```

---

### Task 2: Implement Pages 1-3 (Title + Market Opportunity)

**Files:**
- Modify: `/root/.claude/skills/pptx/create_okinawa_ma_executive.js`

**Steps:**

- [ ] **Step 1: Add Page 1 (Title Slide)**

Append to the script before the final `pres.writeFile()`:
...
- [ ] **Step 2: Add Page 2 (Market Gap Definition)**
...
- [ ] **Step 3: Add Page 3 (Why the Gap Exists)**
...
- [ ] **Step 4: Verify pages 1-3 render correctly**

Run: `cd /root/.claude/skills/pptx && node create_okinawa_ma_executive.js` (this will error at the end since writeFile() isn't added yet, but check that no syntax errors occur up to that point)

- [ ] **Step 5: Commit**

```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add pages 1-3 (title + market opportunity)"
```

---

### Task 3: Implement Pages 4-5 (Competitive Analysis)
...
- [ ] **Step 1: Add Page 4 (Competitor Landscape Table)**
...
tableData.forEach((row, rowIdx) => {
  ...
  row.forEach((cell, colIdx) => {
    ...
    slide.addShape(pres.ShapeType.rect, {
      ...
      fill: { color: bgColor }, line: { color: colors.mediumGray, width: 1 }
    });
...
slide.addText('注：小規模対応 ○=対応可能／△=限定的／×=基本対応せず', {
  ...
  fontSize: 10, color: colors.darkGray, fontFace: 'Arial', align: 'left'
});
```

- [ ] **Step 2: Add Page 5 (Business Model Analysis)**
...
slide.addShape(pres.ShapeType.rect, {
  ...
  fill: { color: 'E8F5E9' }, line: { color: colors.navy, width: 1 }
});
...
```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add pages 4-5 (competitive analysis)"
```

---

### Task 4: Implement Pages 6-7 (Risk Assessment)
...
- [ ] **Step 2: Add Page 7 (Mitigation Strategies)**
...
```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add pages 6-7 (risk assessment + mitigation)"
```

---

### Task 5: Implement Pages 8-9 (Entrepreneur Psychology)
...
- [ ] **Step 1: Add Page 8 (Psychology Framework)**
...
patterns.forEach(pat => {
  ...
  slide.addText(pat.desc, {
    ...
    fontSize: 10, color: colors.darkGray, fontFace: 'Arial'
  });
...
- [ ] **Step 2: Add Page 9 (Decision Triggers)**
...
slide.addShape(pres.ShapeType.rect, {
  ...
  fill: { color: 'E3F2FD' }, line: { color: colors.navy, width: 1 }
});
...
```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add pages 8-9 (entrepreneur psychology)"
```

---

### Task 6: Implement Pages 10-11 (GLOW Strategy)
...
- [ ] **Step 1: Add Page 10 (Differentiation + Revenue Model)**
...
- [ ] **Step 2: Add Page 11 (9-Month Roadmap)**
...
```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add pages 10-11 (GLOW strategy)"
```

---

### Task 7: Implement Page 12 (Conclusion) & Add Write Output
...
- [ ] **Step 2: Add writeFile() at the end of the script**

Append to the end of the script, after all slide definitions:
...
Run: `cd /root/.claude/skills/pptx && node -c create_okinawa_ma_executive.js`
...
```bash
git add /root/.claude/skills/pptx/create_okinawa_ma_executive.js
git commit -m "feat: add page 12 (conclusion) and writeFile"
```

---

### Task 8: Execute Script & Generate PowerPoint

**Files:**
- Output: `/tmp/okinawa_ma_executive.pptx`
...
```bash
cd /root/.claude/skills/pptx && node create_okinawa_ma_executive.js
```

Expected: `Created: /tmp/okinawa_ma_executive.pptx`

- [ ] **Step 2: Verify file was created**

```bash
ls -lh /tmp/okinawa_ma_executive.pptx
```

Expected: File exists, size ~150-200K

---

### Task 9: Copy to Project & Commit

**Files:**
- Copy: `/tmp/okinawa_ma_executive.pptx` → `/home/user/hojo-hq/docs/okinawa_ma_executive.pptx`
...
```bash
cp /tmp/okinawa_ma_executive.pptx /home/user/hojo-hq/docs/
```

- [ ] **Step 2: Verify copy**

```bash
ls -lh /home/user/hojo-hq/docs/okinawa_ma_executive.pptx
```

- [ ] **Step 3: Commit**

```bash
cd /home/user/hojo-hq && git add docs/okinawa_ma_executive.pptx && git commit -m "$(cat <<'EOF'
docs: Add executive presentation - Okinawa M&A market analysis

12-page executive summary for investment decision-making:
- Pages 1-3: Market opportunity (ギャップ市場, ¥300-500B, +50% growth)
- Pages 4-5: Competitive analysis (Okinawa-specific M&A firms, business model)
- Pages 6-7: Risk assessment (Okinawa-specific risks + mitigation strategies)
- Pages 8-9: Entrepreneur psychology (contradiction → patterns → triggers)
- Pages 10-11: GLOW strategy (differentiation, revenue model, 9-month roadmap)
- Page 12: Conclusion (investment decision points)

Data-driven (no hypotheses), Wantedly-style minimalist design 
(white + orange/navy accents), optimized for C-level decision makers.

Co-Authored-By: Claude Haiku 4.5 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01TEzcNeJ3f17VrXKN3gRkrA
EOF
)"
```
...
```bash
git push -u origin claude/okinawa-ma-market-research-mh8ih0
```
...
- [ ] 12 slides created (Pages 1-12)
- [ ] All key content from spec included:
  - [ ] Page 1-3: Market gap + opportunity
  - [ ] Page 4-5: Competitive analysis (Okinawa firms + business models)
  - [ ] Page 6-7: Okinawa-specific risks + mitigation
  - [ ] Page 8-9: Psychology 3-layer analysis (contradiction, patterns, triggers)
  - [ ] Page 10-11: GLOW positioning + 9-month roadmap
  - [ ] Page 12: Investment decision points
- [ ] Design consistent (Wantedly minimalist, white + orange/navy only)
- [ ] All tables/boxes render without overflow
- [ ] File saved to `/home/user/hojo-hq/docs/okinawa_ma_executive.pptx`
