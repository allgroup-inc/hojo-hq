...
test("isStale: 次回アクション予定日が設定されていても、最終接触が長期間無ければ長期検討対象(isOverdueと異なり予定日を優先しない)", () => {
...
test("isStale: 連絡不要・終了ステージの企業は長期検討対象外", () => {
...
test("buildStaleList: 長期検討企業のみ経過日数の降順で返す(直近接触の企業は除外)", () => {
  const records = [
