...
test("buildFillStats: 差し戻し・開始時刻なし・別日は空き扱い(集計しない)", () => {
  const list = [
    apo({ "ステータス": "差し戻し" }),
    apo({ "ステータス": "スケジュール調整中", "開始時刻": "" }),
...
test("buildKindStats: アポ種別ごとに訪問実施数・申込数・申込率を返す(スキーマ順)", () => {
  const list = [
    apo({ "アポ種別": "新規(紹介)", "ステータス": "申込" }),
    apo({ "アポ種別": "新規(紹介)", "ステータス": "申込" }),
    apo({ "アポ種別": "新規(紹介)", "ステータス": "訪問済" }),
    apo({ "アポ種別": "再訪(既存)", "ステータス": "訪問済" }),
    apo({ "アポ種別": "再訪(既存)", "ステータス": "申込" }),
    apo({ "アポ種別": "再訪(既存)", "ステータス": "差し戻し" }),
    apo({ "アポ種別": "新規(ご家族)", "ステータス": "スケジュール調整中" })
  ];
...
test("buildKindStats: 訪問実施ゼロの種別は率null・種別未入力は集計しない・sinceDateで絞れる", () => {
  const rows = core.buildKindStats([
    apo({ "アポ種別": "", "ステータス": "申込" }),
    apo({ "アポ種別": "新規(紹介)", "ステータス": "申込", "日付": "2026-07-01" })
