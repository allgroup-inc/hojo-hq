...
test("診断フロー: 入力→マッチング→結果表示→LINE登録導線", async ({ page }) => {
  ...
  expect(count, `マッチ件数が数値でない: "${countText}"`).not.toBeNaN();

  // 全開放方針(2026-07-23決裁): 全カードが隠されず表示され、公式リンクを持つこと
  const cards = page.locator("#result-list .card");
  expect(await cards.count()).toBeGreaterThan(0);
  await expect(page.locator("#result-list .card.locked")).toHaveCount(0);
  const officialLinks = page.locator('#result-list .card a[href*="http"]');
  expect(await officialLinks.count()).toBe(await cards.count());

  // 価値型CV: 通知価値を訴求するLINE導線が生きていること
  const notifyCta = page.locator("#result-list .unlock a.line-btn");
  await expect(notifyCta).toHaveAttribute("href", /lin\.ee|line\.me/);

  // 申請簡素化: gBizID事前準備ガイドが表示されること
  await expect(page.locator("#result-list .gbizid-note")).toBeVisible();
...
test("全制度一覧: 登録不要で全件閲覧・検索・タグ絞り込みができる", async ({ page }) => {
  await page.goto(SITE);
  await waitForData(page);

  const rows = page.locator("#browse-list .brow");
  await expect(rows.first()).toBeVisible({ timeout: 10000 });
  const initial = await rows.count();
  expect(initial).toBeGreaterThan(0);

  // 各行が公式ページへの直リンクを持つこと
  await expect(rows.first().locator("a")).toHaveAttribute("href", /^https?:\/\//);

  // キーワード検索が機能すること(件数表示が更新される)
  await page.fill("#browse-q", "設備");
  await expect(page.locator("#browse-count")).toContainText(/件|見つかりません/);

  // タグ絞り込みが機能すること
  await page.fill("#browse-q", "");
  await page.locator('.browse-tags .bt[data-t="nationwide"]').click();
  await expect(page.locator('.browse-tags .bt[data-t="nationwide"]')).toHaveClass(/on/);
  await expect(page.locator("#browse-count")).toContainText("件");
});
